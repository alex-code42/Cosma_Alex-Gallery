...
test("All art pieces have a title", async () => {
  useSWR.mockReturnValue({ data: artPieces, isLoading: false, error: false });
  render(<ArtGallery pieces={artPieces} />);
  const list = screen.getByRole("list");
  const { getAllByTestId } = within(list);
  const items = getAllByTestId("title");
  expect(list).toBeInTheDocument();
  for (let i = 0; i < items.length; i++) {
    expect(items[i].innerHTML).toMatch(artPieces[i].name);
  }
});
...
test("All art pieces have a title", async () => {
  ...
  const list = screen.getByRole("list");
  const { getAllByTestId } = within(list);
  const items = getAllByTestId("artist");
  expect(list).toBeInTheDocument();
  for (let i = 0; i < items.length; i++) {
    expect(items[i].innerHTML).toMatch(artPieces[i].artist);
  }
});
